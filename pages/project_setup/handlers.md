 ...
 * `close` - for cleaning up upon `jagql.close()` (optional)
...
```
`resourceConfig` is the complete configuration object passed in to `jagql.define()`.

#### close
`close` is invoked without any parameters, when `jagql.close()` is called.
